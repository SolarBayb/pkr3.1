...
int main()
{
	...
	Print(s, N);
    Count(s, N);
	cout  << "Студенти, які отримали оцінки 5 або 4 з фізики і математики: " << endl;
	Prizv(s, N);
	delete[] s;
	return 0;
}
void Create(Student* s, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		cout << " оцінка з інформатики: "; cin >> s[i].inform;
	}
}
void Print(Student* s, const int N)
{
...
void Count(Student* s, const int N)
{
	...
	cout << "Кількість оцінок задовільно з фізики: " << count1 << endl;
	cout << "Кількість оцінок задовільно з математики: " << count2 << endl;
	cout << "Кількість оцінок задовільно з інформатики: " << count3 << endl;
